...
export const AudioWaveform = forwardRef<HTMLDivElement, AudioWaveformProps>(
  ({ isActive, mode, className, barCount = 12 }, ref) => {
    ...
    return (
      <div ref={ref} className={cn("flex items-end justify-center gap-1 h-16", className)}>
...
export const CircularWaveform = forwardRef<HTMLCanvasElement, CircularWaveformProps>(
  ({ isActive, mode, size = 120, className }, ref) => {
    ...
    const getColor = useCallback(() => {
      switch (mode) {
        case "listening": return "hsl(185, 100%, 55%)";
        case "speaking": return "hsl(210, 100%, 60%)";
        default: return "hsl(210, 100%, 40%)";
      }
    }, [mode]);
    ...
    useEffect(() => {
      ...
      const draw = () => {
        ...
        if (analyserRef.current && audioData) {
          (analyserRef.current as AnalyserNode).getByteFrequencyData(audioData as unknown as Uint8Array<ArrayBuffer>);
        }
        ...
        for (let ring = 0; ring < rings; ring++) {
          ...
          for (let i = 0; i <= segments; i++) {
            ...
            if (audioData && audioData.length > 0 && isActive) {
              ...
              amplitude = ((audioData[dataIndex] ?? 0) / 255) * 15;
            } else if (isActive) {
